...
socket.onmessage = function(event) {
    ...
    if (data.action === "actualizarTurno") {
        actualizarTurno();
    }
};
...
function actualizarTurno() {
    fetch('turno_data.php')
        .then(response => response.json())
        .then(data => {
            document.getElementById("turno-nombre").textContent = data.nombre;
            document.getElementById("turno-puesto").textContent = data.puesto;

            // Habilitar o deshabilitar botones según el turno actual
            document.getElementById("btn-anterior").disabled = (data.turno_actual == 0);
            document.getElementById("btn-siguiente").disabled = (data.turno_actual >= data.total_personas - 1);
        })
        .catch(error => console.error("Error al actualizar turno:", error));
}

// Enviar cambios de turno por WebSocket
document.getElementById("btn-anterior").addEventListener("click", function(event) {
    event.preventDefault();
    socket.send(JSON.stringify({ action: "cambiarTurno", turno: "anterior" }));
});

document.getElementById("btn-siguiente").addEventListener("click", function(event) {
    event.preventDefault();
    socket.send(JSON.stringify({ action: "cambiarTurno", turno: "siguiente" }));
});
</script>
...
$(".btn").click(function(e) {
    ...
    $.post("cambiar_turno.php", { accion: accion }, function(response) {
        if (response.success) {
            actualizarTurno();
        }
    }, "json");
    ...
    <div class="container">
        <div class="turno-box">
            ...
            <?php if ($persona_actual): ?>
                <div class="turno-nombre" id="turno-nombre"><?php echo strtoupper(htmlspecialchars($persona_actual["nombre"] ?? "NO HAY TURNOS")); ?></div>
                <div class="turno-puesto" id="turno-puesto"><?php echo strtoupper(htmlspecialchars($persona_actual["puesto"] ?? "")); ?></div>
            <?php else: ?>
                <div class="turno-nombre">NO HAY TURNOS</div>
            <?php endif; ?>

            <form action="" method="post" class="buttons">
                <button type="submit" name="anterior" id="btn-anterior" class="btn" <?php echo $_SESSION["turno_actual"] == 0 ? 'disabled' : ''; ?>>⬅ Anterior</button>
                <button type="submit" name="siguiente" id="btn-siguiente" class="btn" <?php echo $_SESSION["turno_actual"] >= $total_personas - 1 ? 'disabled' : ''; ?>>Siguiente ➡</button>
            </form>
        </div>
    </div>
